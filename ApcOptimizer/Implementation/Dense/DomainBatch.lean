...
theorem denseAddConstraintDoms_decode [Fact p.Prime] {cs : ConstraintSystem p}
    {bs : BusSemantics p} (reg : VarRegistry) :
    ∀ (dcs : List (DenseExpr p)) (_hcov : ∀ c ∈ dcs, c.CoveredBy reg)
      (hmem : ∀ c ∈ dcs.map reg.decodeExpr, c ∈ cs.algebraicConstraints)
      (Tspec : DomainTable p cs bs) (Tdense : DenseDomainTable p),
      (∀ k, reg.Valid k → Tdense.map[k]? = Tspec.map[reg.resolve k]?) →
      ∀ k, reg.Valid k →
        (denseAddConstraintDoms dcs Tdense).map[k]?
          = (addConstraintDoms (dcs.map reg.decodeExpr) hmem Tspec).map[reg.resolve k]? := by
  intro dcs
  induction dcs with
  | nil => intro _ hmem Tspec Tdense hinv k hk; exact hinv k hk
  | cons c rest ih =>
      intro hcov hmem Tspec Tdense hinv k hk
      have hcc : c.CoveredBy reg := hcov c (List.mem_cons_self ..)
      have hcrest : ∀ c' ∈ rest, c'.CoveredBy reg :=
        fun c' hc' => hcov c' (List.mem_cons_of_mem _ hc')
      have hc : reg.decodeExpr c ∈ cs.algebraicConstraints :=
        hmem (reg.decodeExpr c) (by rw [List.map_cons]; exact List.mem_cons_self ..)
      have hrest : ∀ c' ∈ rest.map reg.decodeExpr, c' ∈ cs.algebraicConstraints :=
        fun c' hc' => hmem c' (by rw [List.map_cons]; exact List.mem_cons_of_mem _ hc')
      have hvd : (reg.decodeExpr c).vars.dedup = (c.vars.dedup).map reg.resolve := by
        rw [reg.decodeExpr_vars c, reg.map_dedup_resolve c.vars (fun i hi => hcc i hi)]
      show (denseAddConstraintDoms rest
              (if (c.vars.dedup).length ≤ 3
               then denseAddConstraintVars c c.vars.dedup Tdense else Tdense)).map[k]?
        = (addConstraintDoms (rest.map reg.decodeExpr) hrest
              (if ((reg.decodeExpr c).vars.dedup).length ≤ 3
               then addConstraintDoms.addVars (reg.decodeExpr c) hc ((reg.decodeExpr c).vars.dedup) Tspec
               else Tspec)).map[reg.resolve k]?
      rw [hvd, List.length_map]
      by_cases hg : (c.vars.dedup).length ≤ 3
      · rw [if_pos hg, if_pos hg]
        refine ih hcrest hrest _ _ ?_ k hk
        exact denseAddConstraintVars_decode reg c hcc hc (c.vars.dedup)
          (fun i hi => hcc i (List.mem_dedup.1 hi)) Tspec Tdense hinv
      · rw [if_neg hg, if_neg hg]
        exact ih hcrest hrest _ _ hinv k hk
...
theorem denseAddBusVars_decode [NeZero p] {cs : ConstraintSystem p} {bs : BusSemantics p}
    (facts : BusFacts p bs) (reg : VarRegistry) (bi : BusInteraction (DenseExpr p))
    (hbc : denseBICovered reg bi) (hbi : reg.decodeBI bi ∈ cs.busInteractions) :
    ∀ (xs : List VarId), (∀ i ∈ xs, reg.Valid i) →
      ∀ (Tspec : DomainTable p cs bs) (Tdense : DenseDomainTable p),
      (∀ k, reg.Valid k → Tdense.map[k]? = Tspec.map[reg.resolve k]?) →
      ∀ k, reg.Valid k →
        (denseAddBusVars bs facts bi xs Tdense).map[k]?
          = (addBusDoms.addVars facts (reg.decodeBI bi) hbi (xs.map reg.resolve) Tspec).map[reg.resolve k]? := by
  intro xs
  induction xs with
  | nil => intro _ Tspec Tdense hinv k hk; exact hinv k hk
  | cons i is ih =>
      ...
      have hiv : reg.Valid i := hxv i (List.mem_cons_self ..)
      have hisv : ∀ i' ∈ is, reg.Valid i' := fun i' hi' => hxv i' (List.mem_cons_of_mem _ hi')
      have hdom := denseInteractionDomainF_decode bs facts reg hiv bi hbc
      show (denseAddBusVars bs facts bi (i :: is) Tdense).map[k]?
        = (addBusDoms.addVars facts (reg.decodeBI bi) hbi
            (reg.resolve i :: is.map reg.resolve) Tspec).map[reg.resolve k]?
      unfold denseAddBusVars addBusDoms.addVars
      split
      · rename_i d hr_d
        have hr_s : interactionDomainF bs facts (reg.decodeBI bi) (reg.resolve i) = some d :=
          hdom.trans hr_d
        split
        · rename_i d' hr_s'
          rw [hr_s] at hr_s'; obtain rfl := Option.some.inj hr_s'
          exact ih hisv _ _
            (DenseDomainTable.insertEntry_decode reg hiv d Tspec Tdense _ hinv) k hk
        · rename_i hr_s'; rw [hr_s] at hr_s'; exact absurd hr_s' (by simp)
      · rename_i hr_d
        have hr_s : interactionDomainF bs facts (reg.decodeBI bi) (reg.resolve i) = none :=
          hdom.trans hr_d
        split
        · rename_i d' hr_s'; rw [hr_s] at hr_s'; exact absurd hr_s' (by simp)
        · exact ih hisv _ _ hinv k hk

theorem denseAddBusDoms_decode [NeZero p] {cs : ConstraintSystem p} {bs : BusSemantics p}
    (facts : BusFacts p bs) (reg : VarRegistry) :
    ∀ (dbis : List (BusInteraction (DenseExpr p))) (_hcov : ∀ bi ∈ dbis, denseBICovered reg bi)
      (hmem : ∀ b ∈ dbis.map reg.decodeBI, b ∈ cs.busInteractions)
      (Tspec : DomainTable p cs bs) (Tdense : DenseDomainTable p),
      (∀ k, reg.Valid k → Tdense.map[k]? = Tspec.map[reg.resolve k]?) →
      ∀ k, reg.Valid k →
        (denseAddBusDoms bs facts dbis Tdense).map[k]?
          = (addBusDoms facts (dbis.map reg.decodeBI) hmem Tspec).map[reg.resolve k]? := by
  intro dbis
  induction dbis with
  | nil => intro _ hmem Tspec Tdense hinv k hk; exact hinv k hk
  | cons bi rest ih =>
      ...
      have hcov' : ∀ b ∈ rest, denseBICovered reg b :=
        fun b hb => hcov b (List.mem_cons_of_mem _ hb)
      ...
      have hvd : (payloadRawVars (reg.decodeBI bi)).dedup
          = ((densePayloadRawVars bi).dedup).map reg.resolve := by
        rw [densePayloadRawVars_decode reg bi,
          reg.map_dedup_resolve (densePayloadRawVars bi) (densePayloadRawVars_valid hbc)]
      show (denseAddBusDoms bs facts (bi :: rest) Tdense).map[k]?
        = (addBusDoms facts (reg.decodeBI bi :: rest.map reg.decodeBI) hmem Tspec).map[reg.resolve k]?
      unfold denseAddBusDoms addBusDoms
      rw [hvd]
      refine ih hcov' hrest _ _ ?_ k hk
      exact denseAddBusVars_decode facts reg bi hbc hbi ((densePayloadRawVars bi).dedup)
        (fun i hi => densePayloadRawVars_valid hbc i (List.mem_dedup.1 hi)) Tspec Tdense hinv

/-! ## The full domain-table correspondence -/

/-- **The dense domain table decodes to the spec domain table under `resolve`** (value-for-value on
    registered ids): building the table from a covered dense system with `denseAddConstraintDoms`
    then `denseAddBusDoms` corresponds to the analogous spec construction on `reg.decodeCS d`.
    `FiniteDomain` values are variable-free, hence *equal*, not decoded. -/
theorem denseDomainTable_decode [Fact p.Prime] [NeZero p] {bs : BusSemantics p}
    (facts : BusFacts p bs) (reg : VarRegistry) (d : DenseConstraintSystem p)
    (hcov : d.CoveredBy reg) :
    ∀ i, reg.Valid i →
      (denseAddBusDoms bs facts d.busInteractions
          (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)).map[i]?
        = (addBusDoms facts (reg.decodeCS d).busInteractions (fun _ h => h)
            (addConstraintDoms (reg.decodeCS d).algebraicConstraints (fun _ h => h)
              (DomainTable.empty (cs := reg.decodeCS d) (bs := bs)))).map[reg.resolve i]? := by
  intro i hi
  have hbase : ∀ k, reg.Valid k →
      (DenseDomainTable.empty : DenseDomainTable p).map[k]?
        = (DomainTable.empty (cs := reg.decodeCS d) (bs := bs)).map[reg.resolve k]? := by
    intro k _; simp [DenseDomainTable.empty, DomainTable.empty]
  have hconstr := denseAddConstraintDoms_decode reg d.algebraicConstraints hcov.1 (fun _ h => h)
    (DomainTable.empty (cs := reg.decodeCS d) (bs := bs)) DenseDomainTable.empty hbase
  exact denseAddBusDoms_decode facts reg d.busInteractions hcov.2 (fun _ h => h) _ _ hconstr i hi
...
/-- Correspondence between a dense `DenseForcedIdx` (over dense system `d` / dense active list
    `dactive`) and a spec `ForcedIdx` over `reg.decodeCS d` / `dactive.map reg.decodeExpr`: item
    arrays are the `toArray`s of the corresponding lists, and the three inverted indices correspond
    (`varless` equal + buckets equal under `resolve`). -/
structure DenseForcedIdxCorr (reg : VarRegistry) (d : DenseConstraintSystem p)
    (dactive : List (DenseExpr p)) (fidx_d : DenseForcedIdx p)
    (fidx_s : ForcedIdx (reg.decodeCS d) (dactive.map reg.decodeExpr)) : Prop where
  arrCs : fidx_d.arrCs = d.algebraicConstraints.toArray
  csVarless : fidx_d.csIdx.varless = fidx_s.csIdx.varless
  csBuckets : ∀ i, reg.Valid i →
    fidx_d.csIdx.buckets.getD i [] = fidx_s.csIdx.buckets.getD (reg.resolve i) []
  arrBis : fidx_d.arrBis = d.busInteractions.toArray
  ...
  arrActive : fidx_d.arrActive = dactive.toArray
  activeVarless : fidx_d.activeIdx.varless = fidx_s.activeIdx.varless
  activeBuckets : ∀ i, reg.Valid i →
    fidx_d.activeIdx.buckets.getD i [] = fidx_s.activeIdx.buckets.getD (reg.resolve i) []

/-! ## Dense `constraintRedundant` (mirrors `constraintRedundant`)

Whether a constraint is identically zero on its own variables' domain box — the redundancy test that
selects the non-redundant `activeCs`. Consumes the dense box `.all` (`denseAllBox`), the compiled
`IExpr` evaluation (shared, variable-free), and the fallback `DenseExpr.eval`. -/
...
/-- Dense `constraintRedundant` (mirrors `constraintRedundant`). -/
def denseConstraintRedundant (T : DenseDomainTable p) (c : DenseExpr p) : Bool :=
  ...
  | some d =>
    (d.map (fun yd => yd.2.size)).prod ≤ maxEnumSize &&
      ...
      | some ic =>
        ...
        let add := addI.add
        let mul := mulI.mul
        denseAllBox (fun a => @decide (denseIExprEvalWith add mul a ic = 0) (dec _ 0)) d
      | none => denseAllBox (fun a => decide (c.eval (denseEnvOfFast a) = 0)) d

/-- **`denseConstraintRedundant` decodes to `constraintRedundant`.** -/
theorem denseConstraintRedundant_decode {cs : ConstraintSystem p} {bs : BusSemantics p}
    (reg : VarRegistry) (Tdense : DenseDomainTable p) (Tspec : DomainTable p cs bs)
    (hT : ∀ k, reg.Valid k → Tdense.map[k]? = Tspec.map[reg.resolve k]?)
    (c : DenseExpr p) (hc : c.CoveredBy reg) :
    constraintRedundant Tspec (reg.decodeExpr c) = denseConstraintRedundant Tdense c := by
  have hvd : (reg.decodeExpr c).vars.dedup = (c.vars.dedup).map reg.resolve := by
    rw [reg.decodeExpr_vars c, reg.map_dedup_resolve c.vars hc]
  have hdedupv : ∀ x ∈ c.vars.dedup, reg.Valid x := fun x hx => hc x (List.mem_dedup.1 hx)
  have hdoms := DenseDomainTable.doms_decode reg Tdense Tspec hT c.vars.dedup hdedupv
  unfold constraintRedundant denseConstraintRedundant
  rw [hvd, hdoms]
  cases hdd : Tdense.doms c.vars.dedup with
  | none => rfl
  | some d_d =>
      simp only [Option.map_some]
      have hsize : ((d_d.map (fun kd => (reg.resolve kd.1, kd.2))).map (fun yd => yd.2.size))
          = d_d.map (fun yd => yd.2.size) := by
        rw [List.map_map]; exact List.map_congr_left (fun kd _ => rfl)
      have hkeysfst : d_d.map Prod.fst = c.vars.dedup :=
        DenseDomainTable.doms_fst Tdense c.vars.dedup d_d hdd
      have hkv : ∀ x ∈ d_d.map Prod.fst, reg.Valid x := by rw [hkeysfst]; exact hdedupv
      have hdv : ∀ kd ∈ d_d, reg.Valid kd.1 :=
        fun kd hkd => hkv kd.1 (List.mem_map.2 ⟨kd, hkd, rfl⟩)
      have hkeysmap : (d_d.map (fun kd => (reg.resolve kd.1, kd.2))).map Prod.fst
          = (d_d.map Prod.fst).map reg.resolve := by
        rw [List.map_map, List.map_map]; exact List.map_congr_left (fun kd _ => rfl)
      have hcomp : compileE ((d_d.map (fun kd => (reg.resolve kd.1, kd.2))).map Prod.fst) (reg.decodeExpr c)
          = denseCompileE (d_d.map Prod.fst) c := by
        rw [hkeysmap]; exact denseCompileE_decode reg (d_d.map Prod.fst) hkv c hc
      rw [hsize, hcomp]
      cases hce : denseCompileE (d_d.map Prod.fst) c with
      | some ic =>
          congr 1
          refine (denseAllBox_decode reg _ _ d_d hdv ?_).symm
          intro ptd hvptd hkeys
          simp only [denseIExprEvalWith_map]
      | none =>
          congr 1
          refine (denseAllBox_decode reg _ _ d_d hdv ?_).symm
          intro ptd hvptd hkeys
          simp only [denseExpr_eval_decode reg c hc ptd hvptd]

/-! ## `forcedOver` as data, and the dense `forcedOver`

`forcedOver` returns dependent `Σ`-pairs carrying entailment witnesses; the pass consumes only the
key/value data (via `collectForced`'s `f.1`/`f.2.val`). `forcedOver_data` extracts that data as a
clean witness-free expression, which the dense `denseForcedOver` mirrors verbatim over `VarId`s. -/

/-- `forcedOver`'s result, projected to its `(variable, value)` data (the witnesses discarded). -/
theorem forcedOver_data {cs : ConstraintSystem p} {bs : BusSemantics p} (facts : BusFacts p bs)
    ...
    (xs : List Variable) :
    (forcedOver facts T activeCs hactiveCs fidx xs).map (fun f => (f.1, f.2.val))
      = match T.doms xs with
        | none => []
        | some fdoms =>
          if (fdoms.map (fun yd => yd.2.size)).prod ≤ maxEnumSize then
            if (!(CoveredIndex.coveredIdxUnord fidx.csIdx fidx.arrCs (fun c => c.varsInF xs) xs).isEmpty
                  || (CoveredIndex.coveredIdxUnord fidx.bisIdx fidx.arrBis
                        (fun bi => bi.varsInF xs && !bs.isStateful bi.busId) xs).any (biInformative bs facts))
                && (fdoms.map (fun yd => yd.2.size)).prod
                    * ((CoveredIndex.coveredIdxUnord fidx.csIdx fidx.arrCs (fun c => c.varsInF xs) xs).length
                       + (CoveredIndex.coveredIdxUnord fidx.bisIdx fidx.arrBis
                            (fun bi => bi.varsInF xs && !bs.isStateful bi.busId) xs).length) ≤ maxEnumWork then
              match scanBox (compiledSurv bs
                    (CoveredIndex.coveredIdxUnord fidx.activeIdx fidx.arrActive (fun c => c.varsInF xs) xs)
                    (CoveredIndex.coveredIdxUnord fidx.bisIdx fidx.arrBis
                      (fun bi => bi.varsInF xs && !bs.isStateful bi.busId) xs)
                    (fdoms.map Prod.fst)).val (fdoms.map Prod.fst) fdoms with
              | none => (fdoms.map Prod.fst).map (fun x => (x, (0 : ZMod p)))
              | some res => res
            else []
          else [] := by
  rw [forcedOver]
  split
  · rename_i hd; simp only [hd, List.map_nil]
  · rename_i fdoms hd
    simp only [hd]
    split_ifs with hbs hw
    · split <;> rename_i heq <;>
        simp [heq, List.map_map, Function.comp_def, List.attach_map_subtype_val]
    · rfl
    · rfl

/-- The dense `forcedOver`: the witness-free data of `forcedOver`, over `VarId`s. -/
def denseForcedOver (bs : BusSemantics p) (facts : BusFacts p bs) (T : DenseDomainTable p)
    (fidx : DenseForcedIdx p) (xs : List VarId) :
    List (VarId × ZMod p) :=
  match T.doms xs with
  | none => []
  | some fdoms =>
    if (fdoms.map (fun yd => yd.2.size)).prod ≤ maxEnumSize then
      if (!(denseCoveredIdxUnord fidx.csIdx fidx.arrCs (fun c => c.varsInF xs) xs).isEmpty
            || (denseCoveredIdxUnord fidx.bisIdx fidx.arrBis
                  (fun bi => denseBIVarsInF xs bi && !bs.isStateful bi.busId) xs).any
                (denseBiInformative bs facts))
          && (fdoms.map (fun yd => yd.2.size)).prod
              * ((denseCoveredIdxUnord fidx.csIdx fidx.arrCs (fun c => c.varsInF xs) xs).length
                 + (denseCoveredIdxUnord fidx.bisIdx fidx.arrBis
                      (fun bi => denseBIVarsInF xs bi && !bs.isStateful bi.busId) xs).length) ≤ maxEnumWork then
        match denseScanBox (denseCompiledSurv bs
              ...
              (fdoms.map Prod.fst))
            (fdoms.map Prod.fst) fdoms with
        | none => (fdoms.map Prod.fst).map (fun x => (x, (0 : ZMod p)))
        | some res => res
      else []
    else []

/-! ### Dense covered-index soundness + the `biInformative` `.any` correspondence -/
...
theorem denseAny_biInformative (reg : VarRegistry) (bs : BusSemantics p) (facts : BusFacts p bs) :
    ∀ (bis : List (BusInteraction (DenseExpr p))), (∀ bi ∈ bis, denseBICovered reg bi) →
      (bis.map reg.decodeBI).any (biInformative bs facts) = bis.any (denseBiInformative bs facts) := by
  intro bis
  induction bis with
  | nil => intro _; rfl
  | cons bi rest ih =>
      intro hcov
      have hbc : denseBICovered reg bi := hcov bi (List.mem_cons_self ..)
      have hcr : ∀ b ∈ rest, denseBICovered reg b := fun b h => hcov b (List.mem_cons_of_mem _ h)
      simp only [List.map_cons, List.any_cons, denseBiInformative_decode reg bs facts bi hbc, ih hcr]

/-! ## `denseForcedOver` decodes to `forcedOver`

The survivor point returned by `denseForcedOver` maps (keys resolved, values equal) to the data of
`forcedOver`'s: the domain box, the covered-set indices, the informativeness/work gates, and the
box scan all correspond. -/

/-- **`denseForcedOver` decodes to `forcedOver` (as data).** -/
theorem denseForcedOver_decode (bs : BusSemantics p) (facts : BusFacts p bs) (reg : VarRegistry)
    (d : DenseConstraintSystem p) (hd : d.CoveredBy reg) (dactive : List (DenseExpr p))
    (hda : ∀ c ∈ dactive, c.CoveredBy reg) (Tdense : DenseDomainTable p)
    (Tspec : DomainTable p (reg.decodeCS d) bs)
    (hT : ∀ k, reg.Valid k → Tdense.map[k]? = Tspec.map[reg.resolve k]?)
    (fidx_d : DenseForcedIdx p) (fidx_s : ForcedIdx (reg.decodeCS d) (dactive.map reg.decodeExpr))
    (hactiveCs : ∀ c ∈ dactive.map reg.decodeExpr, c ∈ (reg.decodeCS d).algebraicConstraints)
    (hfidx : DenseForcedIdxCorr reg d dactive fidx_d fidx_s)
    (xs_d : List VarId) (hxv : ∀ x ∈ xs_d, reg.Valid x) :
    (denseForcedOver bs facts Tdense fidx_d xs_d).map (fun f => (reg.resolve f.1, f.2))
      = (forcedOver facts Tspec (dactive.map reg.decodeExpr) hactiveCs fidx_s (xs_d.map reg.resolve)).map
          (fun f => (f.1, f.2.val)) := by
  rw [forcedOver_data]
  have hdoms := DenseDomainTable.doms_decode reg Tdense Tspec hT xs_d hxv
  unfold denseForcedOver
  rw [hdoms]
  cases hdd : Tdense.doms xs_d with
  | none => simp
  | some fdoms_d =>
    simp only [Option.map_some]
    have hkeysfst : fdoms_d.map Prod.fst = xs_d := DenseDomainTable.doms_fst Tdense xs_d fdoms_d hdd
    have hkvKeys : ∀ x ∈ fdoms_d.map Prod.fst, reg.Valid x := by rw [hkeysfst]; exact hxv
    have hdv : ∀ kd ∈ fdoms_d, reg.Valid kd.1 :=
      fun kd hkd => hkvKeys kd.1 (List.mem_map.2 ⟨kd, hkd, rfl⟩)
    have hesFull : (denseCoveredIdxUnord fidx_d.csIdx fidx_d.arrCs (fun c => c.varsInF xs_d) xs_d).map reg.decodeExpr
        = CoveredIndex.coveredIdxUnord fidx_s.csIdx fidx_s.arrCs
            (fun c => c.varsInF (xs_d.map reg.resolve)) (xs_d.map reg.resolve) := by
      rw [hfidx.arrCs, fidx_s.harrCs]
      exact denseCoveredIdxUnord_corr reg reg.decodeExpr fidx_d.csIdx fidx_s.csIdx d.algebraicConstraints
        (fun c => c.varsInF xs_d) (fun c => c.varsInF (xs_d.map reg.resolve)) xs_d
        (denseCandidates_corr reg fidx_d.csIdx fidx_s.csIdx xs_d hxv hfidx.csVarless hfidx.csBuckets)
        (fun c hc => (denseExpr_varsInF_decode reg xs_d hxv c (hd.1 c hc)).symm)
    have hbis : (denseCoveredIdxUnord fidx_d.bisIdx fidx_d.arrBis
          (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId) xs_d).map reg.decodeBI
        = CoveredIndex.coveredIdxUnord fidx_s.bisIdx fidx_s.arrBis
            (fun bi => bi.varsInF (xs_d.map reg.resolve) && !bs.isStateful bi.busId) (xs_d.map reg.resolve) := by
      rw [hfidx.arrBis, fidx_s.harrBis]
      exact denseCoveredIdxUnord_corr reg reg.decodeBI fidx_d.bisIdx fidx_s.bisIdx d.busInteractions
        (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId)
        (fun bi => bi.varsInF (xs_d.map reg.resolve) && !bs.isStateful bi.busId) xs_d
        (denseCandidates_corr reg fidx_d.bisIdx fidx_s.bisIdx xs_d hxv hfidx.bisVarless hfidx.bisBuckets)
        (fun bi hc => by simp only [denseBIVarsInF_decode reg xs_d hxv bi (hd.2 bi hc), reg.decodeBI_busId])
    have hes : (denseCoveredIdxUnord fidx_d.activeIdx fidx_d.arrActive (fun c => c.varsInF xs_d) xs_d).map reg.decodeExpr
        = CoveredIndex.coveredIdxUnord fidx_s.activeIdx fidx_s.arrActive
            (fun c => c.varsInF (xs_d.map reg.resolve)) (xs_d.map reg.resolve) := by
      rw [hfidx.arrActive, fidx_s.harrActive]
      exact denseCoveredIdxUnord_corr reg reg.decodeExpr fidx_d.activeIdx fidx_s.activeIdx dactive
        (fun c => c.varsInF xs_d) (fun c => c.varsInF (xs_d.map reg.resolve)) xs_d
        (denseCandidates_corr reg fidx_d.activeIdx fidx_s.activeIdx xs_d hxv hfidx.activeVarless hfidx.activeBuckets)
        (fun c hc => (denseExpr_varsInF_decode reg xs_d hxv c (hda c hc)).symm)
    have hbisCov : ∀ bi ∈ denseCoveredIdxUnord fidx_d.bisIdx fidx_d.arrBis
        (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId) xs_d, denseBICovered reg bi :=
      fun bi hbm => hd.2 bi ((denseCoveredIdxUnord_mem_of_eq fidx_d.bisIdx d.busInteractions
        fidx_d.arrBis hfidx.arrBis _ xs_d hbm).1)
    have hesCov : ∀ c ∈ denseCoveredIdxUnord fidx_d.activeIdx fidx_d.arrActive
        (fun c => c.varsInF xs_d) xs_d, c.CoveredBy reg :=
      fun c hcm => hda c ((denseCoveredIdxUnord_mem_of_eq fidx_d.activeIdx dactive
        fidx_d.arrActive hfidx.arrActive _ xs_d hcm).1)
    have hboxlist : (fdoms_d.map (fun kd => (reg.resolve kd.1, kd.2))).map (fun yd => yd.2.size)
        = fdoms_d.map (fun yd => yd.2.size) := by
      rw [List.map_map]; exact List.map_congr_left (fun kd _ => rfl)
    have hkeys : (fdoms_d.map (fun kd => (reg.resolve kd.1, kd.2))).map Prod.fst
        = (fdoms_d.map Prod.fst).map reg.resolve := by
      rw [List.map_map, List.map_map]; exact List.map_congr_left (fun kd _ => rfl)
    simp only [← hesFull, ← hbis, hboxlist, List.isEmpty_map, List.length_map]
    rw [denseAny_biInformative reg bs facts _ hbisCov]
    split_ifs with hc1 hc2
    · rw [← hes, ← hbis, hkeys]
      have hsurv : ∀ (ptd : List (VarId × ZMod p)), (∀ kv ∈ ptd, reg.Valid kv.1) →
          ptd.map Prod.fst = fdoms_d.map Prod.fst →
          denseCompiledSurv bs
              (denseCoveredIdxUnord fidx_d.activeIdx fidx_d.arrActive (fun c => c.varsInF xs_d) xs_d)
              (denseCoveredIdxUnord fidx_d.bisIdx fidx_d.arrBis
                (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId) xs_d)
              (fdoms_d.map Prod.fst) ptd
            = (compiledSurv bs
                ((denseCoveredIdxUnord fidx_d.activeIdx fidx_d.arrActive (fun c => c.varsInF xs_d) xs_d).map reg.decodeExpr)
                ((denseCoveredIdxUnord fidx_d.bisIdx fidx_d.arrBis
                  (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId) xs_d).map reg.decodeBI)
                ((fdoms_d.map Prod.fst).map reg.resolve)).val (ptd.map (fun kv => (reg.resolve kv.1, kv.2))) :=
        fun ptd hvptd hptk => denseCompiledSurv_decode reg bs _ hesCov _ hbisCov
          (fdoms_d.map Prod.fst) hkvKeys ptd hvptd hptk
      have hscanrel := denseScanBox_decode reg
        (denseCompiledSurv bs
          (denseCoveredIdxUnord fidx_d.activeIdx fidx_d.arrActive (fun c => c.varsInF xs_d) xs_d)
          (denseCoveredIdxUnord fidx_d.bisIdx fidx_d.arrBis
            (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId) xs_d)
          (fdoms_d.map Prod.fst))
        (compiledSurv bs
          ((denseCoveredIdxUnord fidx_d.activeIdx fidx_d.arrActive (fun c => c.varsInF xs_d) xs_d).map reg.decodeExpr)
          ((denseCoveredIdxUnord fidx_d.bisIdx fidx_d.arrBis
            (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId) xs_d).map reg.decodeBI)
          ((fdoms_d.map Prod.fst).map reg.resolve)).val
        (fdoms_d.map Prod.fst) fdoms_d rfl hdv hsurv
      revert hscanrel
      cases denseScanBox
          (denseCompiledSurv bs
            (denseCoveredIdxUnord fidx_d.activeIdx fidx_d.arrActive (fun c => c.varsInF xs_d) xs_d)
        ...
        cases scanBox
            (compiledSurv bs
              ...
              ((denseCoveredIdxUnord fidx_d.bisIdx fidx_d.arrBis
                (fun bi => denseBIVarsInF xs_d bi && !bs.isStateful bi.busId) xs_d).map reg.decodeBI)
              ((fdoms_d.map Prod.fst).map reg.resolve)).val
            ((fdoms_d.map Prod.fst).map reg.resolve) (fdoms_d.map (fun kd => (reg.resolve kd.1, kd.2))) <;>
        intro hscanrel
      · simp [Function.comp_def]
      · exact absurd hscanrel (by simp [scanRel])
      · exact absurd hscanrel (by simp [scanRel])
      · obtain ⟨heq, -⟩ := hscanrel; rw [heq]
    · rfl
    · rfl

/-! ## Keys of `denseForcedOver` are valid

Every forced key comes from `T.doms xs`' keys (= `xs`), which are valid: the box scan only ever
tracks values keyed by `keys`, so a survivor point's keys stay within `keys`. -/

/-- A predicate `P` preserved by the fold step is preserved by `foldlStop`. -/
theorem foldlStop_invariant {α β : Type} (P : β → Prop) (f : β → α → β) (stop : β → Bool)
    (hf : ∀ acc a, P acc → P (f acc a)) :
    ∀ (l : List α) (acc : β), P acc → P (foldlStop f stop l acc) := by
  intro l
  induction l with
  ...
  | cons a rest ih =>
      ...
      · rw [if_neg hs]; exact ih (f acc a) (hf acc a h)

/-- A predicate `P` preserved by the fold step is preserved by `FiniteDomain.foldElts`. -/
theorem foldElts_invariant {β : Type} (P : β → Prop) (g : β → ZMod p → β) (stop : β → Bool)
    (hg : ∀ acc v, P acc → P (g acc v)) (d : FiniteDomain p) (acc : β) (h : P acc) :
    P (d.foldElts g stop acc) := by
  rw [FiniteDomain.foldElts_eq]
  exact foldlStop_invariant P g stop hg d.toList acc h

/-- A predicate `P` preserved by the point step is preserved by `denseBoxFold`. -/
theorem denseBoxFold_invariant {β : Type} (P : β → Prop) (f : β → List (VarId × ZMod p) → β)
    (stop : β → Bool) (hf : ∀ acc pt, P acc → P (f acc pt)) :
    ∀ (doms : List (VarId × FiniteDomain p)) (acc : β), P acc → P (denseBoxFold f stop doms acc) := by
  intro doms
  induction doms generalizing f with
  | nil =>
      intro acc h
      show P (if stop acc then acc else f acc [])
      by_cases hs : stop acc = true
      · rw [if_pos hs]; exact h
      · rw [if_neg hs]; exact hf acc [] h
  | cons xd rest ih =>
      intro acc h
      obtain ⟨x, d⟩ := xd
      show P (denseBoxFold (fun acc' a => d.foldElts (fun acc'' v => f acc'' ((x, v) :: a)) stop acc')
        stop rest acc)
      refine ih _ ?_ acc h
      intro acc' a hacc'
      exact foldElts_invariant P _ stop (fun acc'' v hacc'' => hf acc'' ((x, v) :: a) hacc'') d acc' hacc'

/-- Every key of a `denseScanBox` survivor is one of `keys`. -/
theorem denseScanBox_keys (surv : List (VarId × ZMod p) → Bool) (keys : List VarId)
    (doms : List (VarId × FiniteDomain p)) :
    ∀ st, denseScanBox surv keys doms = some st → ∀ kv ∈ st, kv.1 ∈ keys := by
  have hstep : ∀ (acc : Option (List (VarId × ZMod p))) (pt : List (VarId × ZMod p)),
      (∀ st, acc = some st → ∀ kv ∈ st, kv.1 ∈ keys) →
      (∀ st, denseScanStep surv keys acc pt = some st → ∀ kv ∈ st, kv.1 ∈ keys) := by
    intro acc pt hacc
    cases acc with
    | none =>
        intro st hst
        simp only [denseScanStep] at hst
        split at hst
        · injection hst with h; subst h
          intro kv hkv; obtain ⟨x, hx, rfl⟩ := List.mem_map.1 hkv; exact hx
    ...
    | some cands =>
        ...
        split at hst
        · injection hst with h; subst h
          intro kv hkv; exact hacc cands rfl kv (List.mem_filter.1 hkv).1
        · injection hst with h; subst h; exact hacc cands rfl
  exact denseBoxFold_invariant
    (fun o => ∀ st, o = some st → ∀ kv ∈ st, kv.1 ∈ keys)
    (denseScanStep surv keys) denseScanStop hstep doms none (by intro st hst; exact absurd hst (by simp))

/-- Keys of the forced-result match are among `keys`. -/
theorem forcedResult_keys (keys : List VarId) (o : Option (List (VarId × ZMod p)))
    (ho : ∀ st, o = some st → ∀ kv ∈ st, kv.1 ∈ keys) :
    ∀ f ∈ (match o with
        | none => keys.map (fun x => (x, (0 : ZMod p)))
        | some res => res), f.1 ∈ keys := by
  cases o with
  | none => intro f hf; obtain ⟨x, hx, rfl⟩ := List.mem_map.1 hf; exact hx
  | some res => intro f hf; exact ho res rfl f hf

/-- **Every key `denseForcedOver` forces is valid** (it lies in `xs`). -/
theorem denseForcedOver_keys_valid (bs : BusSemantics p) (facts : BusFacts p bs) (reg : VarRegistry)
    (T : DenseDomainTable p) (fidx : DenseForcedIdx p) (xs : List VarId)
    (hxsv : ∀ x ∈ xs, reg.Valid x) :
    ∀ f ∈ denseForcedOver bs facts T fidx xs, reg.Valid f.1 := by
  intro f hf
  unfold denseForcedOver at hf
  split at hf
  · exact absurd hf (by simp)
  · rename_i fdoms hdd
    split at hf
    · split at hf
      · have hkfst : fdoms.map Prod.fst = xs := DenseDomainTable.doms_fst T xs fdoms hdd
        have hmem := forcedResult_keys (fdoms.map Prod.fst) _
          (denseScanBox_keys _ (fdoms.map Prod.fst) fdoms) f hf
        rw [hkfst] at hmem; exact hxsv f.1 hmem
      · exact absurd hf (by simp)
    · exact absurd hf (by simp)

/-! ## `insertAll` preserves `Corr` (for a decode-corresponding pair list)

`collectForced` threads its forced pairs through `Solved.insertAll` / `DenseSolved.insertAll`, whose
`map`/`revDeps` are the folds `Solved.insertAll_map`/`_revDeps`. A raw `CorrData` invariant on the
four underlying `HashMap`s is preserved by each paired insert (`corrData_insert_step`), folds over the
pair list (`corrData_fold`), and lifts back to `Corr` (`corr_insertAll`), given the dense pairs decode
(keys resolved, values decoded) to the spec pairs. Reuses the Gauss generic fold membership lemmas. -/

/-- The `Corr` fields on the raw underlying `HashMap`s (no `Solved`/`DenseSolved` wrappers), so the
    `insertAll` folds can be threaded without constructing intermediate proof-carrying solutions. -/
structure CorrData (reg : VarRegistry) (dm : Std.HashMap VarId (DenseExpr p))
    (drd : Std.HashMap VarId (Std.HashSet VarId)) (sm : Std.HashMap Variable (Expression p))
    (srd : Std.HashMap Variable (Std.HashSet Variable)) : Prop where
  mapVal : ∀ i, reg.Valid i → (dm[i]?).map reg.decodeExpr = sm[reg.resolve i]?
  mapKeyValid : ∀ (i : VarId) (s : DenseExpr p), dm[i]? = some s → reg.Valid i
  mapCov : ∀ (i : VarId) (s : DenseExpr p), dm[i]? = some s → s.CoveredBy reg
  isEmptyEq : dm.isEmpty = sm.isEmpty
  revDeps : ∀ z, reg.Valid z → ∀ k, reg.Valid k →
    (k ∈ ((drd[z]?).getD ∅) ↔ reg.resolve k ∈ ((srd[reg.resolve z]?).getD ∅))

theorem corrData_of_corr (reg : VarRegistry) {cs : ConstraintSystem p} {bs : BusSemantics p}
    {dσ : DenseSolved p} {σ : Solved p cs bs} (hcorr : Corr reg dσ σ) :
    CorrData reg dσ.map dσ.revDeps σ.map σ.revDeps :=
  ⟨hcorr.mapVal, hcorr.mapKeyValid, hcorr.mapCov, hcorr.isEmptyEq, hcorr.revDeps⟩

/-- One paired insert (map insert + revDeps nested-insert) preserves `CorrData`. -/
theorem corrData_insert_step (reg : VarRegistry)
    {dm : Std.HashMap VarId (DenseExpr p)} {drd : Std.HashMap VarId (Std.HashSet VarId)}
    {sm : Std.HashMap Variable (Expression p)} {srd : Std.HashMap Variable (Std.HashSet Variable)}
    (hcd : CorrData reg dm drd sm srd) (x : VarId) (t : DenseExpr p)
    (hxv : reg.Valid x) (htcov : t.CoveredBy reg) :
    CorrData reg (dm.insert x t)
      (t.vars.foldl (fun rd z => rd.insert z (((rd[z]?).getD ∅).insert x)) drd)
      (sm.insert (reg.resolve x) (reg.decodeExpr t))
      ((reg.decodeExpr t).vars.foldl
        (fun rd z => rd.insert z (((rd[z]?).getD ∅).insert (reg.resolve x))) srd) := by
  refine ⟨?_, ?_, ?_, ?_, ?_⟩
  · intro i hi
    ...
    · rw [if_neg (by simpa using hxi),
        if_neg (by simpa using (fun h => hxi (reg.resolve_inj hxv hi h)))]
      exact hcd.mapVal i hi
  · intro i s h
    rw [Std.HashMap.getElem?_insert] at h
    by_cases hxi : x = i
    · exact hxi ▸ hxv
    · rw [if_neg (by simpa using hxi)] at h; exact hcd.mapKeyValid i s h
  · intro i s h
    rw [Std.HashMap.getElem?_insert] at h
    by_cases hxi : x = i
    · rw [if_pos (by simpa using hxi), Option.some.injEq] at h; exact h ▸ htcov
    · rw [if_neg (by simpa using hxi)] at h; exact hcd.mapCov i s h
  · rw [Std.HashMap.isEmpty_insert, Std.HashMap.isEmpty_insert]
  · intro z hz k hk
    rw [revBucket_mem x z k t.vars drd,
      revBucket_mem (reg.resolve x) (reg.resolve z) (reg.resolve k) (reg.decodeExpr t).vars srd]
    have hvarsEq : (z ∈ t.vars) ↔ (reg.resolve z ∈ (reg.decodeExpr t).vars) := by
      rw [reg.decodeExpr_vars]; exact (mem_map_resolve reg hz t.vars htcov).symm
    have hxk : (x = k) ↔ (reg.resolve x = reg.resolve k) :=
      ⟨fun h => by rw [h], fun h => reg.resolve_inj hxv hk h⟩
    rw [hcd.revDeps z hz k hk]
    exact or_congr Iff.rfl (and_congr hvarsEq hxk)

/-- Folding paired inserts over a decode-corresponding pair list preserves `CorrData`. -/
theorem corrData_fold (reg : VarRegistry) :
    ∀ (dpairs : List (VarId × DenseExpr p)), (∀ q ∈ dpairs, reg.Valid q.1) →
      (∀ q ∈ dpairs, q.2.CoveredBy reg) →
      ∀ (dm : Std.HashMap VarId (DenseExpr p)) (drd : Std.HashMap VarId (Std.HashSet VarId))
        (sm : Std.HashMap Variable (Expression p)) (srd : Std.HashMap Variable (Std.HashSet Variable)),
        CorrData reg dm drd sm srd →
        CorrData reg
          (dpairs.foldl (fun m q => m.insert q.1 q.2) dm)
          (dpairs.foldl (fun rd q => q.2.vars.foldl
            (fun rd z => rd.insert z (((rd[z]?).getD ∅).insert q.1)) rd) drd)
          ((dpairs.map (fun q => (reg.resolve q.1, reg.decodeExpr q.2))).foldl
            (fun m q => m.insert q.1 q.2) sm)
          ((dpairs.map (fun q => (reg.resolve q.1, reg.decodeExpr q.2))).foldl
            (fun rd q => q.2.vars.foldl
              (fun rd z => rd.insert z (((rd[z]?).getD ∅).insert q.1)) rd) srd) := by
  intro dpairs
  induction dpairs with
  | nil => intro _ _ dm drd sm srd hcd; exact hcd
  | cons q rest ih =>
      intro hkv hcov dm drd sm srd hcd
      obtain ⟨x, t⟩ := q
      have hxv : reg.Valid x := hkv (x, t) (List.mem_cons_self ..)
      have htcov : t.CoveredBy reg := hcov (x, t) (List.mem_cons_self ..)
      have hkvr : ∀ q ∈ rest, reg.Valid q.1 := fun q h => hkv q (List.mem_cons_of_mem _ h)
      have hcovr : ∀ q ∈ rest, q.2.CoveredBy reg := fun q h => hcov q (List.mem_cons_of_mem _ h)
      simp only [List.map_cons, List.foldl_cons]
      exact ih hkvr hcovr _ _ _ _ (corrData_insert_step reg hcd x t hxv htcov)

/-- **`insertAll` preserves `Corr`** when the dense pairs decode (keys resolved, values decoded) to
    the spec pairs, every dense key is valid, and every dense value is covered. -/
theorem corr_insertAll (reg : VarRegistry) {cs : ConstraintSystem p} {bs : BusSemantics p}
    {dσ : DenseSolved p} {σ : Solved p cs bs} (hcorr : Corr reg dσ σ)
    (dpairs : List (VarId × DenseExpr p)) (hkv : ∀ q ∈ dpairs, reg.Valid q.1)
    (hcov : ∀ q ∈ dpairs, q.2.CoveredBy reg) (σ' : Solved p cs bs)
    (hmap : σ'.map = (dpairs.map (fun q => (reg.resolve q.1, reg.decodeExpr q.2))).foldl
      (fun m q => m.insert q.1 q.2) σ.map)
    (hrev : σ'.revDeps = (dpairs.map (fun q => (reg.resolve q.1, reg.decodeExpr q.2))).foldl
      (fun rd q => q.2.vars.foldl (fun rd z => rd.insert z (((rd[z]?).getD ∅).insert q.1)) rd) σ.revDeps) :
    Corr reg (dσ.insertAll dpairs) σ' := by
  have hcd := corrData_fold reg dpairs hkv hcov dσ.map dσ.revDeps σ.map σ.revDeps
    (corrData_of_corr reg hcorr)
  refine ⟨fun i hi => ?_, fun i s h => ?_, fun i s h => ?_, ?_, fun z hz k hk => ?_⟩
  · rw [DenseSolved.insertAll_map, hmap]; exact hcd.mapVal i hi
  · rw [DenseSolved.insertAll_map] at h; exact hcd.mapKeyValid i s h
  · rw [DenseSolved.insertAll_map] at h; exact hcd.mapCov i s h
  · rw [DenseSolved.insertAll_map, hmap]; exact hcd.isEmptyEq
  · rw [DenseSolved.insertAll_revDeps, hrev]; exact hcd.revDeps z hz k hk

/-! ## Dense `collectForced` and its bisimulation

`denseVarSetKey` deliberately materializes the resolved `Variable`s and reuses the spec `varSetKey`,
so the dedup `HashSet String` keys are *identical* strings to the spec's — a cold-path key computation
that keeps the dedup behaviour byte-identical. `denseCollectForced` mirrors `collectForced`'s
structural recursion, threading a `DenseSolved` and the shared `HashSet String`; the bisimulation
`denseCollectForced_corr` carries `Corr` through, using `denseForcedOver_decode` for the forced pairs
and `corr_insertAll` for the per-target `insertAll`. -/
...
/-- Dense `collectForced` (mirrors `collectForced`): collect forced constants over each target's
    variable set, skipping already-seen sets, threading a `DenseSolved`. -/
def denseCollectForced (bs : BusSemantics p) (facts : BusFacts p bs) (reg : VarRegistry)
  ...
  | xs :: rest, seen, dσ =>
    ...
    if seen.contains key then denseCollectForced bs facts reg T fidx rest seen dσ
    else denseCollectForced bs facts reg T fidx rest (seen.insert key)
      (dσ.insertAll ((denseForcedOver bs facts T fidx xs).map (fun f => (f.1, DenseExpr.const f.2))))

/-- **The dense `collectForced` corresponds (`Corr`) to the spec `collectForced`** on decoding
    targets, given a covered dense system / active list, corresponding tables and inverted indices,
    and *identical* `seen` string sets. -/
theorem denseCollectForced_corr (bs : BusSemantics p) (facts : BusFacts p bs) (reg : VarRegistry)
    (d : DenseConstraintSystem p) (hd : d.CoveredBy reg) (dactive : List (DenseExpr p))
    (hda : ∀ c ∈ dactive, c.CoveredBy reg) (Tdense : DenseDomainTable p)
    (Tspec : DomainTable p (reg.decodeCS d) bs)
    (hT : ∀ k, reg.Valid k → Tdense.map[k]? = Tspec.map[reg.resolve k]?)
    ...
    ∀ (targets_d : List (List VarId)) (targets_s : List (List Variable)),
      ...
      ∀ (seen : Std.HashSet String) (dσ : DenseSolved p) (σ : Solved p (reg.decodeCS d) bs),
        Corr reg dσ σ →
        Corr reg (denseCollectForced bs facts reg Tdense fidx_d targets_d seen dσ)
          (collectForced facts Tspec (dactive.map reg.decodeExpr) hactiveCs fidx_s targets_s seen σ) := by
  intro targets_d
  induction targets_d with
  | nil =>
      intro targets_s htgt _ seen dσ σ hcorr
      subst htgt
      exact hcorr
  | cons xs_d rest_d ih =>
      intro targets_s htgt hvalid seen dσ σ hcorr
      subst htgt
      have hxsv : ∀ x ∈ xs_d, reg.Valid x := hvalid xs_d (List.mem_cons_self ..)
      have hvalidr : ∀ xs ∈ rest_d, ∀ x ∈ xs, reg.Valid x :=
        fun xs h => hvalid xs (List.mem_cons_of_mem _ h)
      simp only [List.map_cons, denseCollectForced, collectForced, denseVarSetKey]
      split_ifs with hc
      · exact ih (rest_d.map (fun xs => xs.map reg.resolve)) rfl hvalidr seen dσ σ hcorr
      · refine ih (rest_d.map (fun xs => xs.map reg.resolve)) rfl hvalidr _ _ _ ?_
        have hdo := denseForcedOver_decode bs facts reg d hd dactive hda Tdense Tspec hT
          fidx_d fidx_s hactiveCs hfidx xs_d hxsv
        have hpairs : ((forcedOver facts Tspec (dactive.map reg.decodeExpr) hactiveCs fidx_s
              (xs_d.map reg.resolve)).map (fun f => (f.1, Expression.const f.2.val)))
            = ((denseForcedOver bs facts Tdense fidx_d xs_d).map
                (fun f => (f.1, DenseExpr.const f.2))).map
                (fun q => (reg.resolve q.1, reg.decodeExpr q.2)) := by
          rw [List.map_map]
          have h2 := congrArg (List.map (fun q : Variable × ZMod p =>
            (q.1, (Expression.const q.2 : Expression p)))) hdo
          rw [List.map_map, List.map_map] at h2
          exact h2.symm
        refine corr_insertAll reg hcorr
          ((denseForcedOver bs facts Tdense fidx_d xs_d).map (fun f => (f.1, DenseExpr.const f.2)))
          ?_ ?_ _ ?_ ?_
        · intro q hq
          obtain ⟨f, hf, rfl⟩ := List.mem_map.1 hq
          exact denseForcedOver_keys_valid bs facts reg Tdense fidx_d xs_d hxsv f hf
        · intro q hq
          obtain ⟨f, hf, rfl⟩ := List.mem_map.1 hq
          exact DenseExpr.coveredBy_const reg f.2
        · rw [Solved.insertAll_map, hpairs]
        · rw [Solved.insertAll_revDeps, hpairs]

/-! ## The dense `domainBatch` pass (Task 3 — DomainBatch, part 4)

The `VarId`-native mirror of `domainBatchPass`: build the domain table, collect every forced constant
from the joint enumerations (`denseCollectForced`), and — unless nothing was forced — substitute the
whole solution map through the system in one traversal. Prime `p` only (the same `pw` gates both the
dense and the spec pass, so the branches align); identity otherwise. The pass inherits
`domainBatchPass`'s `PassCorrect` via the `Corr` bisimulation `denseDomainBatch_corr`, exactly as the
Gauss pass does. -/

/-! ### Bus-interaction `vars` decode + validity (the constraint-side lemmas already exist) -/

/-- The variables of a decoded bus interaction are the resolved dense `denseBIVars` (mirrors
    `decodeExpr_vars` at the bus level). -/
theorem VarRegistry.decodeBI_vars (reg : VarRegistry) (bi : BusInteraction (DenseExpr p)) :
    (reg.decodeBI bi).vars = (denseBIVars bi).map reg.resolve := by
  simp only [VarRegistry.decodeBI, BusInteraction.vars, denseBIVars, List.map_append,
    List.map_flatMap, List.flatMap_map, reg.decodeExpr_vars]

/-- Every `VarId` of a covered dense bus interaction is valid. -/
theorem denseBIVars_valid {reg : VarRegistry} {bi : BusInteraction (DenseExpr p)}
    (hc : denseBICovered reg bi) : ∀ v ∈ denseBIVars bi, reg.Valid v := by
  intro v hv
  simp only [denseBIVars, List.mem_append, List.mem_flatMap] at hv
  obtain ⟨hm, hp⟩ := hc
  rcases hv with h | ⟨e, he, hve⟩
  · exact hm v h
  · exact hp e he v hve

/-! ### The spec pass's data as reusable helpers

`domainBatchPass` builds its domain table / target list / active set / inverted index inline. These
helpers reproduce those terms verbatim (each is *definitionally equal* to the corresponding inline
term the pass builds), so `denseDomainBatch_corr`'s spec side and `domainBatchPass_out_prime` share
one canonical form that lines up with the pass by `rfl`. -/

/-- The spec domain table `domainBatchPass` builds (mirrors the inline `T`). -/
def domainBatchT [Fact p.Prime] [NeZero p] {bs : BusSemantics p} (facts : BusFacts p bs)
    (cs : ConstraintSystem p) : DomainTable p cs bs :=
  addBusDoms facts cs.busInteractions (fun _ h => h)
    (addConstraintDoms cs.algebraicConstraints (fun _ h => h) DomainTable.empty)

/-- The spec target list `domainBatchPass` builds (mirrors the inline `targets`). -/
def domainBatchTargets (cs : ConstraintSystem p) : List (List Variable) :=
  cs.algebraicConstraints.map (fun e => e.vars.dedup) ++
    cs.busInteractions.map (fun bi => bi.vars.dedup)

/-- The spec non-redundant active constraint set (mirrors the inline `activeCs`). -/
def domainBatchActiveCs [Fact p.Prime] [NeZero p] {bs : BusSemantics p} (facts : BusFacts p bs)
    (cs : ConstraintSystem p) : List (Expression p) :=
  cs.algebraicConstraints.filter (fun c => !constraintRedundant (domainBatchT facts cs) c)

/-- The spec inverted index `domainBatchPass` builds (mirrors the inline `fidx`), as a function of
    the active-constraint set. -/
def specForcedIdx (cs : ConstraintSystem p) (activeCs : List (Expression p)) :
    ForcedIdx cs activeCs :=
...
/-- The spec solution map `domainBatchPass` builds (mirrors the inline `σ`; the pass then branches on
    `σ.map.isEmpty`). Definitionally equal to the pass's inline `σ`. -/
def domainBatchσ [Fact p.Prime] [NeZero p] {bs : BusSemantics p} (facts : BusFacts p bs)
    (cs : ConstraintSystem p) : Solved p cs bs :=
  collectForced facts (domainBatchT facts cs) (domainBatchActiveCs facts cs)
    (fun _ h => (List.mem_filter.1 h).1)
    (specForcedIdx cs (domainBatchActiveCs facts cs)) (domainBatchTargets cs) ∅ Solved.empty

/-- `collectForced` depends on its `activeCs` only through the inverted index built from it; two calls
    over decode-equal active sets (with the canonical `specForcedIdx`) coincide (proof arguments are
    irrelevant). -/
theorem collectForced_specForcedIdx_eq {cs : ConstraintSystem p} {bs : BusSemantics p}
    (facts : BusFacts p bs) (T : DomainTable p cs bs) (A1 A2 : List (Expression p)) (hA : A1 = A2)
    (h1 : ∀ c ∈ A1, c ∈ cs.algebraicConstraints) (h2 : ∀ c ∈ A2, c ∈ cs.algebraicConstraints)
    (targets : List (List Variable)) (seen : Std.HashSet String) (σ : Solved p cs bs) :
    collectForced facts T A1 h1 (specForcedIdx cs A1) targets seen σ
      = collectForced facts T A2 h2 (specForcedIdx cs A2) targets seen σ := by
  subst hA; rfl

/-- **`domainBatchPass`'s output on prime `p`.** The `.out` reduces to `if …isEmpty then cs else
    cs.substF …fn`, with the pass's inline `σ` folded into `domainBatchσ` (definitionally equal). -/
theorem domainBatchPass_out_prime [Fact p.Prime] [NeZero p] {pw : PrimeWitness p}
    (hpB : pw.isPrime = true) (cs : ConstraintSystem p) (bs : BusSemantics p)
    (facts : BusFacts p bs) :
    (domainBatchPass pw cs bs facts).out
      = if (domainBatchσ facts cs).map.isEmpty then cs else cs.substF (domainBatchσ facts cs).fn := by
  simp only [domainBatchPass, dif_pos hpB]
  rw [apply_ite PassResult.out]
  rfl

/-! ### The dense transform -/

/-- The dense solution map (mirrors the spec pass's `σ`), built with `denseCollectForced`. Computed
    once per pass invocation and threaded to `applyσ`. Needs `reg` for `denseVarSetKey`. -/
def denseDomainBatchσ (reg : VarRegistry) (bs : BusSemantics p) (facts : BusFacts p bs)
  ...
  let activeCs := d.algebraicConstraints.filter (fun c => !denseConstraintRedundant T c)
  ...
  denseCollectForced bs facts reg T fidx targets ∅ DenseSolved.empty
...
theorem decodeCS_applyσ (reg : VarRegistry) (dσ : DenseSolved p) (d : DenseConstraintSystem p) :
    reg.decodeCS (applyσ dσ d)
      = if dσ.map.isEmpty then reg.decodeCS d else reg.decodeCS (d.substF dσ.fn) := by
  unfold applyσ; rw [apply_ite reg.decodeCS]

/-- The dense domain-batch transform, mirroring `domainBatchPass` exactly. -/
def denseDomainBatchF (pw : PrimeWitness p) (reg : VarRegistry) (bs : BusSemantics p)
    (facts : BusFacts p bs) (d : DenseConstraintSystem p) : DenseConstraintSystem p :=
  if pw.isPrime = true then applyσ (denseDomainBatchσ reg bs facts d) d else d

/-! ### The `Corr` bisimulation

`denseDomainBatch_corr` establishes that the dense solution map corresponds to the spec pass's, by
discharging `denseCollectForced_corr`'s hypotheses (table decode, active-set decode, inverted-index
correspondence, target-list decode) and folding the result onto the canonical `domainBatchσ`. Its
proof mirrors `denseGaussElim_corr`. -/

theorem denseDomainBatch_corr [Fact p.Prime] [NeZero p] (reg : VarRegistry) (bs : BusSemantics p)
    (facts : BusFacts p bs) (d : DenseConstraintSystem p) (hcov : d.CoveredBy reg) :
    Corr reg (denseDomainBatchσ reg bs facts d) (domainBatchσ facts (reg.decodeCS d)) := by
  -- Dense domain table + its decode correspondence.
  have hT := denseDomainTable_decode facts reg d hcov
  -- The dense active-constraint set decodes to the spec one.
  have hAE : (d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
        (denseAddBusDoms bs facts d.busInteractions
          (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c)).map reg.decodeExpr
      = domainBatchActiveCs facts (reg.decodeCS d) := by
    apply filter_map_comm reg.decodeExpr
    intro c hc
    show (!denseConstraintRedundant _ c) = !constraintRedundant (domainBatchT facts (reg.decodeCS d)) (reg.decodeExpr c)
    rw [denseConstraintRedundant_decode reg _ (domainBatchT facts (reg.decodeCS d)) hT c (hcov.1 c hc)]
  -- The dense target list decodes to the spec one.
  have hTE : (d.algebraicConstraints.map (fun e => e.vars.dedup)
        ++ d.busInteractions.map (fun bi => (denseBIVars bi).dedup)).map (fun xs => xs.map reg.resolve)
      = domainBatchTargets (reg.decodeCS d) := by
    unfold domainBatchTargets
    rw [List.map_append]
    congr 1
    · show (d.algebraicConstraints.map (fun e => e.vars.dedup)).map (fun xs => xs.map reg.resolve)
        = (d.algebraicConstraints.map reg.decodeExpr).map (fun e => e.vars.dedup)
      rw [List.map_map, List.map_map]
      refine List.map_congr_left (fun e he => ?_)
      show (e.vars.dedup).map reg.resolve = (reg.decodeExpr e).vars.dedup
      rw [reg.map_dedup_resolve e.vars (hcov.1 e he), reg.decodeExpr_vars]
    · show (d.busInteractions.map (fun bi => (denseBIVars bi).dedup)).map (fun xs => xs.map reg.resolve)
        = (d.busInteractions.map reg.decodeBI).map (fun bi => bi.vars.dedup)
      rw [List.map_map, List.map_map]
      refine List.map_congr_left (fun bi hbi => ?_)
      show ((denseBIVars bi).dedup).map reg.resolve = (reg.decodeBI bi).vars.dedup
      rw [reg.map_dedup_resolve (denseBIVars bi) (denseBIVars_valid (hcov.2 bi hbi)),
        reg.decodeBI_vars]
  -- Every target is a list of valid ids.
  have htgtValid : ∀ xs ∈ (d.algebraicConstraints.map (fun e => e.vars.dedup)
        ++ d.busInteractions.map (fun bi => (denseBIVars bi).dedup)), ∀ x ∈ xs, reg.Valid x := by
    intro xs hxs x hx
    rw [List.mem_append] at hxs
    rcases hxs with h | h
    · obtain ⟨e, he, rfl⟩ := List.mem_map.1 h
      exact hcov.1 e he x (List.mem_dedup.1 hx)
    · obtain ⟨bi, hbi, rfl⟩ := List.mem_map.1 h
      exact denseBIVars_valid (hcov.2 bi hbi) x (List.mem_dedup.1 hx)
  -- The three inverted-index correspondences.
  ...
  have hbiC := denseCovBuild_corr reg reg.decodeBI denseBIVars BusInteraction.vars
    (fun bi => reg.decodeBI_vars bi) d.busInteractions
    (fun bi hbi => denseBIVars_valid (hcov.2 bi hbi))
  have hacC := denseCovBuild_corr reg reg.decodeExpr DenseExpr.vars Expression.vars
    (fun a => reg.decodeExpr_vars a)
    (d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
      (denseAddBusDoms bs facts d.busInteractions
        (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c))
    (fun a ha => hcov.1 a (List.mem_of_mem_filter ha))
  -- Membership of the dense active set decodes into `cs.algebraicConstraints`.
  have hactiveCs : ∀ c ∈ (d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
        (denseAddBusDoms bs facts d.busInteractions
          (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c)).map reg.decodeExpr,
      c ∈ (reg.decodeCS d).algebraicConstraints := by
    intro c hc
    obtain ⟨c0, hc0, rfl⟩ := List.mem_map.1 hc
    exact List.mem_map.2 ⟨c0, List.mem_of_mem_filter hc0, rfl⟩
  -- The forced-index correspondence.
  have hfidx : DenseForcedIdxCorr reg d
      (d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
        (denseAddBusDoms bs facts d.busInteractions
          (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c))
      { csIdx := denseCovBuild DenseExpr.vars d.algebraicConstraints,
        ...
        bisIdx := denseCovBuild denseBIVars d.busInteractions,
        arrBis := d.busInteractions.toArray,
        activeIdx := denseCovBuild DenseExpr.vars (d.algebraicConstraints.filter
          (fun c => !denseConstraintRedundant
            (denseAddBusDoms bs facts d.busInteractions
              (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c)),
        arrActive := (d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
          (denseAddBusDoms bs facts d.busInteractions
            (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c)).toArray }
      (specForcedIdx (reg.decodeCS d)
        ((d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
          (denseAddBusDoms bs facts d.busInteractions
            (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c)).map reg.decodeExpr)) :=
    { arrCs := rfl, csVarless := hcsC.1, csBuckets := hcsC.2,
      arrBis := rfl, bisVarless := hbiC.1, bisBuckets := hbiC.2,
      arrActive := rfl, activeVarless := hacC.1, activeBuckets := hacC.2 }
  -- Apply the capstone bisimulation, then fold onto the canonical spec `σ`.
  have hcorr0 := denseCollectForced_corr bs facts reg d hcov
    (d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
      (denseAddBusDoms bs facts d.busInteractions
        (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c))
    ...
    (denseAddBusDoms bs facts d.busInteractions
      (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty))
    (domainBatchT facts (reg.decodeCS d)) hT
    { csIdx := denseCovBuild DenseExpr.vars d.algebraicConstraints,
      arrCs := d.algebraicConstraints.toArray,
      bisIdx := denseCovBuild denseBIVars d.busInteractions,
      arrBis := d.busInteractions.toArray,
    ...
    (d.algebraicConstraints.map (fun e => e.vars.dedup)
      ++ d.busInteractions.map (fun bi => (denseBIVars bi).dedup))
    _ rfl htgtValid ∅ DenseSolved.empty Solved.empty (corr_empty reg)
  have hconv : collectForced facts (domainBatchT facts (reg.decodeCS d))
        ((d.algebraicConstraints.filter (fun c => !denseConstraintRedundant
          (denseAddBusDoms bs facts d.busInteractions
            (denseAddConstraintDoms d.algebraicConstraints DenseDomainTable.empty)) c)).map reg.decodeExpr)
        hactiveCs
        ...
        ((d.algebraicConstraints.map (fun e => e.vars.dedup)
          ++ d.busInteractions.map (fun bi => (denseBIVars bi).dedup)).map (fun xs => xs.map reg.resolve))
        ∅ Solved.empty
      = domainBatchσ facts (reg.decodeCS d) := by
    rw [hTE]
    unfold domainBatchσ
    exact collectForced_specForcedIdx_eq facts (domainBatchT facts (reg.decodeCS d))
      _ (domainBatchActiveCs facts (reg.decodeCS d)) hAE _
      (fun _ h => (List.mem_filter.1 h).1) (domainBatchTargets (reg.decodeCS d)) ∅ Solved.empty
  rw [hconv] at hcorr0
  exact hcorr0

/-! ### The pass -/

/-- **The dense domain-batch pass.** Built directly (facts are threaded, so `ofTransform` does not
    apply); it inherits `domainBatchPass`'s `PassCorrect` through the `Corr` bisimulation
    `denseDomainBatch_corr`, exactly as `denseGaussElimPass` does. -/
def denseDomainBatchPass (pw : PrimeWitness p) : DenseVerifiedPassW p := fun reg d hcov bs facts =>
  { reg' := reg
    out := denseDomainBatchF pw reg bs facts d
    derivs := []
    ext := VarRegistry.Extends.refl reg
    covered := by
      simp only [denseDomainBatchF]
      by_cases hpB : pw.isPrime = true
      · rw [if_pos hpB]
        haveI : Fact p.Prime := ⟨pw.correct hpB⟩
        haveI : NeZero p := ⟨(pw.correct hpB).ne_zero⟩
        have hcorr := denseDomainBatch_corr reg bs facts d hcov
        unfold applyσ
        by_cases he : (denseDomainBatchσ reg bs facts d).map.isEmpty = true
        · rw [if_pos he]; exact hcov
        · rw [if_neg he]
          exact DenseConstraintSystem.substF_covered hcov (fun i _ t ht => hcorr.mapCov i t ht)
      · rw [if_neg hpB]; exact hcov
    dcovered := by intro x hx; simp at hx
    correct := by
      show PassCorrect (reg.decodeCS d) (reg.decodeCS (denseDomainBatchF pw reg bs facts d))
        (reg.decodeDerivs ([] : DenseDerivations p)) bs
      have hout : reg.decodeCS (denseDomainBatchF pw reg bs facts d)
          = (domainBatchPass pw (reg.decodeCS d) bs facts).out := by
        by_cases hpB : pw.isPrime = true
        · haveI : Fact p.Prime := ⟨pw.correct hpB⟩
          haveI : NeZero p := ⟨(pw.correct hpB).ne_zero⟩
          have hcorr := denseDomainBatch_corr reg bs facts d hcov
          have hLHS : denseDomainBatchF pw reg bs facts d
              = applyσ (denseDomainBatchσ reg bs facts d) d := by
            simp only [denseDomainBatchF, if_pos hpB]
          rw [hLHS, decodeCS_applyσ, domainBatchPass_out_prime hpB (reg.decodeCS d) bs facts,
            hcorr.isEmptyEq]
          by_cases he : (domainBatchσ facts (reg.decodeCS d)).map.isEmpty = true
          · rw [if_pos he, if_pos he]
          · rw [if_neg he, if_neg he]
            exact reg.decodeCS_substF (denseDomainBatchσ reg bs facts d).fn
              (domainBatchσ facts (reg.decodeCS d)).fn (fun i hi => hcorr.mapVal i hi) d hcov
        · have hLHS : denseDomainBatchF pw reg bs facts d = d := by
            simp only [denseDomainBatchF, if_neg hpB]
          rw [hLHS]
          simp only [domainBatchPass, dif_neg hpB]
      rw [show reg.decodeDerivs ([] : DenseDerivations p) = [] from rfl, hout]
      have hderiv : (domainBatchPass pw (reg.decodeCS d) bs facts).derivs = [] := by
        simp only [domainBatchPass]
        split
        · split <;> rfl
        · rfl
      have hc := (domainBatchPass pw (reg.decodeCS d) bs facts).correct
      rw [hderiv] at hc
      exact hc }
