...
The single-drop correctness step of `busPairCancel`, over the dense `VarId` representation: dropping
one matched consecutive send/receive pair (optionally emitting replacement byte checks) is a
`DensePassCorrect` step. Every semantic notion is the dense one from `Dense/Bridge.lean`
(`DenseConstraintSystem.satisfies`/`admissible`/`sideEffects`/`guaranteesInvariants`/`implies`,
`denseBIEval`), every `BusFacts` field is applied at the *value* level over `denseBIEval`, and the
byte obligation is discharged from `denseRecvSlotsJustified_sound`
(`BusPairCancelJustifyProof.lean`).

## Statement shape

`denseDropPair_correct` is stated over a **plain** constraint system with the split hypothesis
`d.busInteractions = A ++ S :: B ++ R :: C`, concluding
`DensePassCorrect isInput d { d with busInteractions := A ++ B ++ C ++ checks } [] bs` — assembled
via `DensePassCorrect.ofEnvEq`. The `mkCs` projection is *not* part of this statement: the
`mkCs`-shaped intermediate systems are threaded only later, in `BusPairCancel.lean`'s
`denseCancelLoop`, which instantiates this lemma's `cs`/`d` with `mkCs cs0 arr alive checks` and
recognises the output via `denseLiveSeg_split`/`denseLiveSeg_drop`. So `denseCancelLoop` composes the
single steps this file provides through `DensePassCorrect.andThen`, and `denseCheckCancel`
(`BusPairCancelCheck.lean`) feeds this lemma its hypotheses: payload equality (`hpayEval`,
discharged via a consecutive-payload entailment), the between-region refutation (`hmidEval`) and
before-region shield (`hpreEval`) (discharged via `denseMidRefuted`/`denseShieldOk`), and the byte
obligation (`hbyte`).

## `denseToBusState`/`denseActiveStatefulMsgs` are proof-only

Neither is called by any runtime pass transform — the runtime produces the output constraint system
directly. They are free-form proof-side helpers here, and equal
`DenseConstraintSystem.sideEffects`/`admissible` (from `Dense/Bridge.lean`) definitionally on
`d.busInteractions`. -/
...
`denseToBusState` is the stateful-bus side-effect state of a raw dense interaction list, which is
exactly what `DenseConstraintSystem.sideEffects` computes on `d.busInteractions`. `multiplicitySum`
and its additivity `multiplicitySum_append` live over the representation-independent `BusState p`. -/
...
`denseActiveStatefulMsgs` is the active, stateful evaluated messages of a raw dense interaction
list — the argument dense `admissible` passes to `bs.admissible`. Proof-only; equals
`DenseConstraintSystem.admissible`'s argument on `d.busInteractions` definitionally. -/
...
/-- A split of the active∧stateful evaluated messages of `A` lifts to a syntactic split of `A`
    ...
    `admissible_dropPair` shield stated over `denseActiveStatefulMsgs`. -/
...
/-- **Correctness of dropping one matched consecutive send/receive pair, optionally emitting
    replacement byte checks.** `S` (a send) and `R` (a later receive) carry the same evaluated
    payload (`hpayEval`), are on a bus `busId` with a declared `shape` and a `recvByteSlots` contract
    whose byte obligation for `R` is justified by the remaining interactions *including the emitted
    checks* (`hbyte`, discharged by `denseRecvSlotsJustified_sound`), with no active same-address
    message between them (`hmidEval`) and every active same-address send before `S` shielded by a
    later active same-address receive (`hpreEval`). Each emitted check is stateless, implied by `R`'s
    own accepted receive, invariant-free, and adds no `VarId`s (`hchecks`). Dropping the pair and
    appending the checks is `DensePassCorrect`. Assembled via `DensePassCorrect.ofEnvEq`, so
    `denseCancelLoop` composes the single steps through `DensePassCorrect.andThen`. -/
