...
Everything here is purely equational — no field/primality assumptions, and it quantifies over
`Variable`/`Expression`/`ConstraintSystem` spec values only. It is neutral shared infrastructure:
`DomainProp.lean` consumes `ConstraintSystem.subst` / `subst_correct` from here directly. -/
