...
2. **drop** the now-entailed checks via `DensePassCorrect.denseFilterBusEntailed`
   (`FlagFoldDropsProof.lean`), the native mirror of `filterBusEntailed_correct`; the added
   booleanity survives the bus filter (it touches only interactions), so the dropped check's
   acceptance is re-derived from the still-present constraint.
...
/-! ## Dense booleanity eval / vars (native mirrors of `ZeroWidthRange.boolC_eval`/`_vars`) -/
...
/-- Structure of a recognised width-1 check (native mirror of `RangeBool.boolCheck?_spec`). -/
...
/-- A recognised width-1 check lives on a stateless bus (native mirror of
    `RangeBool.boolCheck?_stateless`). -/
...
/-- For a recognised width-1 check, acceptance is exactly the booleanity of its value variable
    (native mirror of `RangeBool.boolCheck?_violates_iff`). -/
...
/-- The variables of a recognised booleanity occur in `d` (native mirror of
    `RangeBool.boolCheck?_spec`'s variable clause). -/
...
/-- Step 1: adding the recognised booleanities is native `DensePassCorrect`. -/
...
/-- Step 2: dropping the now-entailed checks is native `DensePassCorrect`. -/
