...
/-! # Collapsing a multi-limb reciprocal-witness group to one hint (finishes powdr's `seqz`)

After `ZeroRegister.lean` pins `x0 = 0`, a `beqz`/`bnez` is left with the gadget
`cmp·(cmp−1)=0`, `(cmp−1)·aᵢ=0`, and one *bilinear* constraint `Σᵢ aᵢ·dimᵢ = cmp` carrying **four**
fresh inverse-hint witnesses `dimᵢ` (the `diff_inv_marker` limbs). powdr keeps only **one** inverse
hint. This pass performs that collapse, and it is a *general* transformation, not tied to the
gadget: whenever a constraint is `Σᵢ aᵢ·dimᵢ + t = 0` with

* each `dimᵢ` a **distinct variable occurring exactly once in the whole system** (a pure witness),
* each coefficient `aᵢ` a **single byte-bounded variable** (via `BusFacts.slotBound`), with the
  number of terms small enough that `Σ aᵢ.val < p` cannot wrap,
* `t` and the `aᵢ` reading only input (powdr-ID) columns,

it is replaced by `(Σᵢ aᵢ)·inv + t = 0` with a single fresh derived witness
`inv = QuotientOrZero(−t, Σᵢ aᵢ)`, dropping every `dimᵢ` (−(n−1) variables).

Correctness (discharged via `ConstraintSystem.reencode_correct_D`, `rw = fun x => if x = E then E'
else x`): **soundness** sets every `dimᵢ := inv` (so `Σ aᵢ·dimᵢ = (Σaᵢ)·inv = −t`); **completeness**
computes `inv` by `QuotientOrZero`, the `Σaᵢ = 0` branch closing because byte-bounded coefficients
summing to `0` are all `0`, forcing `t = 0`. Neither touches a bus, so side effects and `admissible`
are unchanged. With `BusFacts.trivial` (no `slotBound`) it never fires. -/
...
theorem collapse_correct [Fact p.Prime] (cs : ConstraintSystem p) (bs : BusSemantics p)
    ...
    (hEeq : ∀ (env : Variable → ZMod p) (w : ZMod p),
      E.eval (reassign D w env) = denom.eval env * w + rest.eval env)
  ...
  have hframe_ne : ∀ (c : Expression p), (∀ d ∈ D, d ∉ c.vars) →
      ∀ (w : ZMod p) (env : Variable → ZMod p), c.eval (reassign D w env) = c.eval env := by
    intro c hc w env
    refine Expression.eval_congr c _ _ (fun x hx => ?_)
    show (if x ∈ D then w else env x) = env x
    rw [if_neg (fun hxD => hc x hxD hx)]
  ...
  have hbe : ∀ (env : Variable → ZMod p) (bi : BusInteraction (Expression p)),
      bi ∈ cs.busInteractions → bi.eval (reassign D (env inv) env) = bi.eval env := by
    intro env bi hbi
  ...
  have hcssat : ∀ env, out.satisfies bs env → cs.satisfies bs (reassign D (env inv) env) := by
    intro env hsat
  ...
  have hsound : out.implies cs bs := by
    intro env hsat
    refine ⟨reassign D (env inv) env, hcssat env hsat, ?_⟩
    have hse : cs.sideEffects bs (reassign D (env inv) env) = out.sideEffects bs env := by
  ...
  · -- invariant preservation
    ...
    exact hcsInv (reassign D (env inv) env) (hcssat env hsatOut) bi hbiOut
...
def tryOne [Fact p.Prime] (cs : ConstraintSystem p) (bs : BusSemantics p)
  ...
  · simp only [Bool.and_eq_true, decide_eq_true_eq] at hchk
    ...
    have hden_pow : ∀ x ∈ (sumExpr (peel D E).1).vars, x.powdrId?.isSome = true := by
      ...
      exact hpow x hxc
    exact some ⟨_, _, collapse_correct cs bs E (sumExpr (peel D E).1) (peel D E).2 D inv hE
