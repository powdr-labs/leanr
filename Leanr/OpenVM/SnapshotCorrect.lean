...
ADD-immediate provably `refines` the input (sound, and complete for the input's intended
executions), and preserves invariants — a direct instance of the general
`optimizer_maintainsCorrectness`.
...
/-- The optimized ADD-immediate circuit `refines` the input under the OpenVM bus semantics:
    sound, and complete for the input's intended (real-trace) executions. -/
theorem addiOptimized_refinesInput :
    addiOptimized.refines addiInput (openVmBusSemantics babyBear) :=
