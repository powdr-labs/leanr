...
/-- The memory discipline transfers across simultaneous substitution. -/
theorem ConstraintSystem.memoryDiscipline_substF (cs : ConstraintSystem p)
    (f : String → Option (Expression p)) (bs : BusSemantics p) (a : String → ZMod p) :
    (cs.substF f).memoryDiscipline bs a ↔ cs.memoryDiscipline bs (envF f a) := by
  unfold ConstraintSystem.memoryDiscipline
  constructor
  · intro h busId shape hdecl
    have hmsgs := h busId shape hdecl
    rwa [ConstraintSystem.msgs_substF] at hmsgs
  · intro h busId shape hdecl
    rw [ConstraintSystem.msgs_substF]
...
theorem ConstraintSystem.satisfies_substF (cs : ConstraintSystem p)
  ...
  have hdisc := cs.memoryDiscipline_substF f bs a
