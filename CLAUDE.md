...
- **`Leanr/Spec.lean`** — Georg's from-scratch spec of circuit equivalence, and the audited contract (extended 2026-07 with Georg's sign-off: `MemoryBusShape`/`memoryBus` + the `memoryDiscipline` conjunct of `satisfies` = the audited last-write-wins assumption, now with a **timestamp-uniqueness** clause and an **empty-address** instantiation for the execution bridge; and `DegreeBound` + `Expression.degree` + `ConstraintSystem.withinDegree` + the `optimizerRespectsDegree` property — the zkVM's multiplicative-degree limit, `BusSemantics.degreeBound`). Models algebraic constraints *and* bus interactions: `BusSemantics` (stateful/stateless, `violatesConstraint`, `breaksInvariant`, `memoryBus`, `degreeBound`), stateful side effects compared up to net multiplicity (`≈` on `BusState`), `optimizerMaintainsCorrectness` (equivalence + invariant preservation) and `optimizerRespectsDegree`. All reusable lemmas and the optimizer live under `Optimizer.lean`/`OptimizerPasses/`, not here.
- **The optimizer, in three layers** (public signature fixed at `ConstraintSystem p → BusSemantics p → ConstraintSystem p` for the tooling/snapshot):
    ...
    - **`Leanr/OptimizerPasses/*.lean`** — reusable proven cores and one file per pass. Cores: `Subst.lean` (`ConstraintSystem.subst_correct` — substituting `x := t` when satisfying assignments force `x = t`; plus the `substFromConstraint` solver→pass combinator), `SubstMap.lean` (**batch** counterpart: `substF` substitutes a whole map in one traversal, `substF_correct`), `Rewrite.lean` (`mapExpr_correct` for eval-preserving rewrites, `filterConstraints_correct`, `filterBus_correct`), `Affine.lean` (`linearize`/`LinExpr`, `solveAffine` with `±1`-preferring *unit*-coefficient pivots, the proven `pm1PivotsOf`/`unitPivotsOf` candidate enumerations), `Normalize.lean` (term-merging `LinExpr.norm`), `TautoBus.lean` (`filterBusStateless_correct`), `MonicScale.lean` (`mapConstraintsIff_correct`; monic scaling with checked unit certificates). Batch passes (each collects *all* its deductions per cycle and applies one `substF`): `Gauss.lean` (`gaussElimPass` — Gaussian elimination against a resolved, proof-carrying `Solved` hash map, duplication-cost pivots with a penalty for rewriting raw stateless payload slots), `DomainBatch.lean` (`domainBatchPass` — proof-carrying `DomainTable`; **joint enumeration** of each target's domain box against all constraints/obligations covered by its variable set), `ExecChain.lean` (`execChainPass` — anchored-maximum unification for the empty-address execution bridge: an anchor send payload-refuted against every receive is the timestamp maximum, every other refuted send is consumed in-fragment, giving the `pc/ts` links between instructions), `MemoryUnifyBatch.lean` (`memoryUnifyBatchPass` — pair send/receive unification; consumes the discipline's **monotonicity clause** to refute candidate consumer receives listed after the successor send `S'` — by monotonicity `tsVal ≥ tsVal S' > tsVal S`, but the consumer carries `S`'s timestamp — which disambiguates the match when a block ends in a computed jump whose terminal re-read of the same register sits on a separate, un-chained `from_state`, entry 36), `ChainUnify.lean` (`chainUnifyPass` — whole same-address chain in one invocation via a descending induction with a claimed-receive accumulator), `Reencode.lean` (`reencodePass` — **witness re-encoding**: a variable group restricted to `m` joint values is replaced by `⌈log₂ m⌉` fresh booleans via degree-aware subexpression interpolation, a *new* correctness core `reencode_correct` transporting witnesses both ways; `interpOf` emits a bare **constant** when the interpolated subexpression is pattern-independent, so a flag-independent register index stays literally constant and keeps memory-chaining rather than being obfuscated into a `Σ indicator·c` polynomial — entry 37). `MemoryUnify.lean`'s `BoundsMap` precomputes fact-derived variable bounds once per pass for the refutation certificates. Single-step predecessors (`ConstantSubst`, `affineSubstPass`, `domainPropPass`, `memoryUnifyPass`) remain as reference, out of the pipeline. `DigitEq.lean` (`digitEqPass`, bounded-decomposition digit matching) is proven and verified but not wired (fires on no OpenVM case). Field-agnostic except the domain/reencode passes (runtime `p.Prime` gate).
    - **`Leanr/Optimizer.lean`** — the thin assembly: `cleanupCycle` = `gauss → normalize → fold → domainBatch → normalize → fold → drop-trivial → drop-zero-mult → drop-tauto-lookup → execChain → chainUnify → memoryUnifyBatch → reencode`, every pass wrapped in `.guardDegree` (checked output, identity fallback), run via `VerifiedPassW.iterateStable` (fixpoint stop; `iters` default 32), then monic canonicalization + fold; `optimizerWith`/`optimizer` as before, with `optimizer_maintainsCorrectness` **and** `optimizer_respectsDegree` (both projections/folds of the carried proofs).
    - **To add an optimization:** create a `VerifiedPass` in a new `Leanr/OptimizerPasses/` file, import it in `Optimizer.lean`, and `.andThen …|>.guardDegree` it into `cleanupCycle`. Do not touch `Spec.lean` or the glue in `Basic.lean`. **Effectiveness: 36/11 ≈ 3.27** on the ADD-immediate snapshot; on the top-100 openvm-eth set **2.54× aggregate / 2.67× geomean** (`log.md` entry 32), all outputs within the degree bound, vs powdr 4.32×. The remaining gap is one lever — cross-instruction timestamp chaining — blocked because blocks end in a control-flow op with a **symbolic** target pc (no anchor for `ExecChain`), and the discipline is deliberately *order-free*; closing it needs an audited **program-order timestamp-monotonicity** assumption (a spec decision for Georg, log entry 32). `DESIGN-bus-knowledge.md` describes the knowledge architecture: proven `BusFacts` (`Leanr/BusFacts.lean`, OpenVM in `Leanr/OpenVM/Facts.lean`) feed fact-domains and `violatesConstraint` probing; the audited `memoryBus` declaration licenses memory/exec unification.
