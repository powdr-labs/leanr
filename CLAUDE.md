...
- **`Leanr/Spec.lean`** — Georg's from-scratch spec of circuit equivalence, and the **frozen surface**: the *definitions* here (see the header block) are the contract and must not change in meaning. Models algebraic constraints *and* bus interactions: `BusSemantics` (stateful/stateless, `violatesConstraint`, `breaksInvariant`), stateful side effects compared up to net multiplicity (`≈` on `BusState`), and `optimizerMaintainsCorrectness` (equivalence + invariant preservation). All reusable lemmas and the optimizer now live in `Optimizer.lean`, not here.
- **`Leanr/Optimizer.lean`** — the circuit optimizer `optimizer` (public signature fixed at `ConstraintSystem p → BusSemantics p → ConstraintSystem p` for the tooling/snapshot), with proof `optimizer_maintainsCorrectness`. Provides the **compositional, proof-carrying framework** for adding optimizations: full equivalence-relation glue (`BusState.equiv_{refl,symm,trans}`, `ConstraintSystem.implies_{refl,trans}`, `equivalentTo_{refl,symm,trans}`); `PassCorrect` (the per-step obligation: equivalent-to-input + invariant-preserving); `VerifiedPass` (a pass bundled with its `PassCorrect` proof — un-gameable, since the proof *is* the return value); `VerifiedPass.andThen` (compose passes via `equivalentTo_trans`); and `pipeline` (the sequence of passes, currently just `VerifiedPass.id`). **To add an optimization:** write a `VerifiedPass` and `.andThen` it into `pipeline`; do not touch `Spec.lean` or the frozen glue. Currently the identity (effectiveness 1) — the "optimizer that does nothing, proven correct" milestone, now on a foundation built to grow.
- **`Leanr/Spec/Dsl.lean`** — reusable, zkVM-agnostic sugar for the spec types: `Add/Mul/Neg/Sub` and `OfNat` instances on `Expression` (write `V "x" * (V "x" - 1)`, numeric literals lower to field constants; `-`/negation lower to `·* -1`), plus a precedence-aware, bus-grouped `render : ConstraintSystem p → String` and a `matchesSnapshot` helper for `#guard`s.
...
- **`Leanr/OpenVM/Snapshot.lean`** — a snapshot test mirroring powdr's `apc_builder_single_instructions`. Ports the constraint system that is the *input* to powdr's `optimize()` for a single OpenVM ADD-immediate (powdr `single_add_1`; 36 cols / 20 bus interactions / 32 constraints), runs `optimizer` on it, and `#guard`s the *output's* rendered string against a stored snapshot plus the invariant `effectiveness ≥ 1` (never grows the circuit), reporting the measured shrink factor via `#eval`. Makes no assumption that the optimizer does nothing, so it survives the optimizer starting to do real work (regenerate the snapshot then). Build/verify with `lake build Leanr.OpenVM.Snapshot` (see note on the root aggregator below).
- **`Leanr/Solver.lean`, `Leanr/Solver/Proofs.lean`, and other pre-existing code** — written by Chris. Treat as reference/prior art, not as the foundation. The plan is to build the spec independently and incorporate Chris's code later where it makes sense, rather than extending it directly.
- **Note:** the root aggregator `Leanr.lean` currently does not build — `Leanr.Spec` and the older `Leanr.Expression` both declare a top-level `Expression`/`Expression.eval`, so importing both clashes. This predates the OpenVM module; build individual spec-based modules directly (e.g. `lake build Leanr.OpenVM.Snapshot`) until the old modules are dropped from the root or namespaced.
